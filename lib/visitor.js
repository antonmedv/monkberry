function Visitor(callback) {
  this.callback = callback;
}

var HTMLElements = (
'a abbr address area article aside audio b base bdi bdo big blockquote body br ' +
'button canvas caption cite code col colgroup data datalist dd del details dfn ' +
'dialog div dl dt em embed fieldset figcaption figure footer form h1 h2 h3 h4 h5 ' +
'h6 head header hr html i iframe img input ins kbd keygen label legend li link ' +
'main map mark menu menuitem meta meter nav noscript object ol optgroup option ' +
'output p param picture pre progress q rp rt ruby s samp script section select ' +
'small source span strong style sub summary sup table tbody td textarea tfoot th ' +
'thead time title tr track u ul var video wbr'
).split(' ');

var SVGElements = (
'circle clipPath defs ellipse g line linearGradient mask path pattern polygon polyline ' +
'radialGradient rect stop svg text tspan'
).split(' ');

Visitor.prototype.visit = function (nodes) {
  var children = [];
  for (var node of nodes) {
    if (node.type === 'tag') {

      if (HTMLElements.indexOf(node.name) != -1) {
        children.push(this.callback.tagNode(node));
      } else if (node.name === 'if') {
        children.push(this.callback.ifNode(node));
      } else if (node.name === 'for') {
        children.push(this.callback.forNode(node));
      } else {
        children.push(this.callback.customNode(node));
      }

    } else if (node.type === 'text') {

      children.push(this.callback.textNode(node));

    } else {
      throw new Error('Unknown node type: "' + node.type + '".');
    }
  }
  return children;
};

module.exports = Visitor;
