...
Compiler.prototype.compileVariables = function (input, callback) {
  ...
  if (textParser.variables.length == 1) {
    ...
    this.addSetters(variable.name, callback(this.compileText(textParser.textParts, [variable.toString()])));
    ...
    for (variable of textParser.variables) {
      ...
      this.addSetters(variable.name, valueName + " = " + variable.toString());
    }
