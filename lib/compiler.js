...
/**
 * Note what for local vars Monkberry uses θ and λ which are valid ECMAScript 5.1 variable names.
 */
...
Compiler.prototype.compileTemplate = function () {
  ...
  if (this.updateActions.length > 0) {
    ...
    code += '  view._update = function (data) {\n';
...
Compiler.prototype.textNode = function (node) {
  ...
  var textParser = this.compileExpression(node.data, function (code) {
...
Compiler.prototype.compileAttributes = function (nodeName, attributes) {
  ...
  Object.keys(attributes).forEach(function (key) {
    if (key == 'id') {
      var id = _this.compileExpression(attributes[key], function (code) {
        return nodeName + ".id = " + code;
      });
      _this.construct.push(nodeName + ".id = " + esc(id.rawText()) + ";");
    } else {
      var attr = _this.compileExpression(attributes[key], function (code) {
...
Compiler.prototype.compileExpression = function (input, callback) {
  var splitter = new Splitter(input);

  if (splitter.variables.length == 1) {
    this.onSetter(splitter.variables[0]).add(callback(splitter.toCode()));
  } else if (splitter.variables.length > 1) {
    var complexSetter = this.onComplexSetter(splitter.variables);
    complexSetter.add(callback(splitter.toCode()));

    for (variable of splitter.variables) {
      this.onSetter(variable).cache();
      this.onSetter(variable).addComplex(splitter.variables, complexSetter.name);
    }
  }
...
Compiler.prototype.ifNode = function (node) {
  ...
  var variable = new ExpressionParser(node.attribs.test);
  ...
  this.addSetters(variable.name,
    ...
    templateName + "', " +
    "data, " +
    "data." + variable.name + // TODO: Implement custom JavaScript in <if> tag.
    ")"
  );
