...
Compiler.prototype.compileUpdate = function () {
  ...
  Object.keys(this.variables).forEach(function (varName) {
...
Compiler.prototype.ifNode = function (node) {
  ...
  var templateName = this.name + '.if_' + variable.name + this.uniqid('template_name');
  var childrenName = 'children' + this.uniqid('children_name');
  if (node.parent) {
    var nodeName = node.parent.nodeName;
  } else {
    throw new Error("If tag must has a parent node.");
  }

  this.subTemplates.push(new Compiler(templateName, node.children));
  this.declarations.push(childrenName + " = {}");
  this.variables[variable.name] = variable;
  this.addSetters(variable.name, "monkberry.foreach(view, " + nodeName + ", " + childrenName + ", '" + templateName + "', [" + variable.getFrom('value') + "])");
...
Compiler.prototype.forNode = function (node) {
  ...
  var templateName = this.name + '.for_' + variable.name + this.uniqid('template_name');
  var childrenName = 'children' + this.uniqid('children_name');
  if (node.parent) {
    var nodeName = node.parent.nodeName;
  } else {
    throw new Error("For tag must has a parent node.");
  }
